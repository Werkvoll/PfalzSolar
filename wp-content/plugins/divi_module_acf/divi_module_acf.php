					...
					$et_pt_acf_table_classes[] = $module_class;
							...
							if (function_exists('get_field')) {
								if ($field = get_field_object($this->shortcode_atts['field_name'])) {
									...
									if ($value) {
										$output = '<tr>
												<td class="sb_mod_acf_table_item clearfix ' . esc_attr( $module_class ) . '">
												' . $title . '
												</td>
												<td>' . $value . '</td>
											    </tr>';
				...
				function shortcode_callback( $atts, $content = null, $function_name ) {
					...
					if (function_exists('get_field')) {
						if ($field = get_field_object($this->shortcode_atts['field_name'])) {
							...
							if (is_array($value) && $field['type'] == 'image') {
								$value = '<a href="' . $value['sizes']['large'] . '" class="sb-divi-acf-single-image-item"><img src="' . (@$value['sizes'][$image_size] ? $value['sizes'][$image_size]:$value['sizes']['large']) . '" /></a>';
							} else if (is_array($value) && $field['type'] == 'gallery') {
								$value_cache = $value;
								$value = '';
								foreach ($value_cache as $val) {
					...
					if ($field) {
						...
						if (isset($field['value']) && is_array($field['value'])) {
							...
							foreach ($value as $key=>$val) {
							    ...
							    $output .= '<td class="sb_mod_acf_table_item clearfix">' . $val . '</td>';
							}
